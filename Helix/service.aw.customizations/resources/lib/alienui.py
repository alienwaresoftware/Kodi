...
class AlienUI:
    def __init__(self):
        currentAlienUIVersion = self.getCurrentAlienUIVersion()
        newAlienUIVersion = self.getNewAlienUIVersion()
        
        if(currentAlienUIVersion and newAlienUIVersion):
            if (cmp(newAlienUIVersion,currentAlienUIVersion) > 0):
                log(('Updating AlienUI version'))
                self.installAlienUI(False)
                return
        elif not currentAlienUIVersion and newAlienUIVersion:
            log(('Installing new AlienUI version'))
            self.installAlienUI(True)
            return
        elif not newAlienUIVersion:
            log(('AlienUI alienuisetup.exe not found'))
            return

        log(('Latest AlienUI version installed'))

    def getCurrentAlienUIVersion(self):
        key = None
        try:   
            key = _winreg.OpenKey(_winreg.HKEY_LOCAL_MACHINE, "SOFTWARE\\AlienUI", 0, _winreg.KEY_READ | _winreg.KEY_WOW64_64KEY)
            version = _winreg.QueryValueEx(key, "Version")[0]            
            versionAsNumbers = [int(numeric_string) for numeric_string in version.split('.')]
            log(('Current AlienUI version = %d.%d.%d.%d') % (versionAsNumbers[0], versionAsNumbers[1], versionAsNumbers[2], versionAsNumbers[3]))
    ...
    def getNewAlienUIVersion(self):
        try:
            self.setupPath = os.path.join(__addonpath__,'resources','lib','alienuisetup.exe')
            log(('AlienUI setup path = %s') % self.setupPath)
            version = self.getFileVersion(self.setupPath)
            log(('New AlienUI version = %d.%d.%d.%d') % (version[0], version[1], version[2], version[3]))
            return version;
        except:
            pass
    ...
    def installAlienUI(self,isFresh):
        message = ''
        if(isFresh):
            if(dialog_yesno(localize(33000),localize(33001))):
                ...
                try:
                    ...
                    log(('AlienUI setup launcher path = %s') % scriptPath)
                    si = subprocess.STARTUPINFO()
                    si.dwFlags |= subprocess.STARTF_USESHOWWINDOW
                    exitCode = subprocess.call(["cscript.exe", scriptPath, self.setupPath], startupinfo=si)
                    log('AlienUI setup exit code = ' + str(tohex(exitCode, 32)))
            ...
            if(dialog_yesno(localize(33007),localize(33008))):
                ...
                log(('AlienUI setup path = %s') % setupPath)

                retValue = AlphaUIUtils.LaunchAlienUISetup(setupPath)
                log('AlienUI setup return code = ' + str(retValue))
