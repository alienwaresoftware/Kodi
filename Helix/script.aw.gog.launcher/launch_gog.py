...
if (__name__ == "__main__"):
    if (xbmcgui.Window(10004).getProperty('service.aw.customizations.isComponentInstalled') == "False"):
        ...
        try:
            ...
            exeName = readRegistryValue(r'SOFTWARE\GOG.com\GalaxyClient', 'clientExecutable')
            exePath = readRegistryValue(r'SOFTWARE\GOG.com\GalaxyClient\paths', 'client')
            launchPath = os.path.join(exePath,exeName)

            if (not exeName or not exePath or not os.path.isfile(launchPath)):
                if (xbmcgui.Dialog().yesno(__addonname__,__customizationaddonlanguage__(33014),'',__customizationaddonlanguage__(33015))):
                    AlphaUIUtils.EnableDisableControllerMouse(True)
                    webbrowser.open('http://www.gog.com')
            else:
